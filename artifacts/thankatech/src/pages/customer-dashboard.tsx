...
import { Heart, Gift, Clock, CheckCircle, ArrowRight } from "lucide-react";
...
export function CustomerDashboard() {
  const { data: jobs, isLoading: isJobsLoading } = useListJobs(
    ...
    { query: { enabled: true, queryKey: getListJobsQueryKey({ customerId: DEMO_CUSTOMER_ID }) } }
  );
  
  const { data: points, isLoading: isPointsLoading } = useGetPoints(DEMO_CUSTOMER_ID, {
    query: { enabled: true, queryKey: getGetPointsQueryKey(DEMO_CUSTOMER_ID) }
  });
      ...
      <div className="container mx-auto max-w-5xl space-y-8">
        <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage your jobs and send gratitude</p>
          </div>
